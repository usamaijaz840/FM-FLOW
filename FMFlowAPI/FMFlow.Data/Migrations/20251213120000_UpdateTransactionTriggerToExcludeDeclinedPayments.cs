...
{
    /// <inheritdoc />
    public partial class UpdateTransactionTriggerToExcludeDeclinedPayments : Migration
    {
        /// <inheritdoc />
            ...
            migrationBuilder.Sql(@"
                ...
                BEGIN
                    ...
                    SET 
                        ...
                        ""DepositHasBeenPaid"" = (
                            CASE
                                WHEN sums.""TotalDebit"" > 0 AND sums.""TotalCredit"" >= (sums.""TotalDebit"" * 0.3)
                                THEN TRUE
                                ELSE FALSE
                            END
                        )
                    FROM (
                        WITH sums AS (
                            SELECT 
                                ""EstimateId"",
                                -- Only sum Credit transactions that are Settled or Approved (exclude Declined)
                                COALESCE(SUM(CASE WHEN ""Status"" IN ('Settled', 'Approved') THEN ""Credit"" ELSE 0 END), 0) AS ""TotalCredit"",
                                -- Sum all Debit transactions (should only be one now, but sum for safety)
                                COALESCE(SUM(""Debit""), 0) AS ""TotalDebit""
                            FROM ""Transactions""
                            WHERE ""EstimateId"" = NEW.""EstimateId""
                            GROUP BY ""EstimateId""
                        )
