...
resource "aws_secretsmanager_secret_version" "keycloak_db" {
  ...
  secret_string = jsonencode({
    ...
    # Full JDBC URL for Keycloak
    url      = "jdbc:postgresql://${aws_db_instance.postgresql.address}:${var.db_port}/${var.keycloak_db_name}"
  })
}

# Note: Security group rule for ECS tasks to access RDS already exists in networking.tf
# Keycloak uses the same ECS tasks security group, so no additional rule needed
...
resource "aws_ecs_task_definition" "keycloak" {
  ...
  "value": "false"
        ...
        # Proxy configuration for ALB - tells Keycloak it's behind a proxy that terminates SSL
        # Using "edge" mode means Keycloak will trust X-Forwarded-Proto header from ALB
        {
          name  = "KC_PROXY"
          value = "edge"
        },
        ...
        # Hostname configuration - let Keycloak auto-detect HTTPS from proxy headers
        {
          name  = "KC_HOSTNAME"
          value = "identity.${var.domain_name}"
        },
        {
      ...
      # Start Keycloak in standard mode
      # After first successful startup and database initialization, you can switch to optimized mode
      # Realm files can be imported by mounting them to /opt/keycloak/data/import/ and adding --import-realm
      command = [
        "start"
      ]
      logConfiguration = {
...
resource "aws_lb_target_group" "keycloak" {
  ...
  health_check {
    ...
    port                = "traffic-port"
    protocol            = "HTTP"
    timeout             = 10  # Increased timeout for Keycloak startup
    unhealthy_threshold = 5   # Increased threshold to allow more time for startup
  }
...
# ALB Listener Rule for Keycloak - Route by host header (identity subdomain)
# This rule has priority 50, which is higher than the default rule (lower number = higher priority)
...
# ALB Listener Rule for Keycloak - Route by path pattern (fallback for path-based routing)
# This rule has priority 100, which should be higher than the default rule
